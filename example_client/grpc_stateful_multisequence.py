...
def CalculateUtteranceError(referenceArray, resultArray):
    ...
    minRef = np.amin(referenceArray)
    return rootMeanErr

def sequenceThread(sample_name, args, batch_size, samples_limit, utterances, scores_map, errors_queue, sequence_id):
    cw_l = 17
    cw_r = 12
    ...
    for x in range(0, batch_size):
        ...
        if x < cw_l:
            inputArray = np.expand_dims(utterances[0], axis=0)
            ivectorsArr = np.expand_dims(ivectors[sample_name][0], axis=0)
        elif x >= cw_l and x < batch_size + cw_l:
            inputArray = np.expand_dims(utterances[x-cw_l], axis=0)
            ivectorsArr = np.expand_dims(ivectors[sample_name][x-cw_l], axis=0)
        else:
            inputArray = np.expand_dims(utterances[batch_size - 1], axis=0)
            ivectorsArr = np.expand_dims(ivectors[sample_name][batch_size - 1], axis=0)
        ...
        if(score_index >= 0):
            referenceArray = scoreObjects[sample_name][score_index]
...
for key, obj in ark_reader:
    for i in range(10):
        ...
        printDebug('\tInput data type: {}'.format(obj.dtype))

        samples_limit = int(args.get('samples'))
        sequence_thread = threading.Thread(target=sequenceThread,
            args=(key, args, batch_size, samples_limit, obj, scoreObjects, errors_queue, sequence_id))
        sequence_thread.start()
        sequences_threads.append(sequence_thread)
        sequence_id += 1


meanGlobalErr = 0.0
for sequence_thread in sequences_threads:
    sequence_thread.join()
while True:
    try:
        meanAvgErr = errors_queue.get_nowait()
        meanGlobalErr = meanGlobalErr + meanAvgErr
    except queue.Empty:
        break
meanGlobalErrAvg = meanGlobalErr/min(numberOfKeys, utterances_limit)
